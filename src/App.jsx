...
const App = () => {
  ...
  return (
    <div className="flex justify-center h-screen bg-gray-100 overflow-hidden font-sans text-black">
      <div className="w-full max-w-[400px] bg-white h-full relative flex flex-col shadow-2xl overflow-hidden">
        ...
        {currentGym && (
          <div className={`absolute inset-0 bg-white z-[1000] transition-transform duration-300 ${activePage === 'detail' || activePage === 'workout' ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            <div className="p-5 overflow-y-auto h-full pb-32 text-black">
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-5 text-center">
                ...
                <button onClick={startTimer} className="w-full bg-black text-white py-4 rounded-xl font-bold">📍 지금 여기서 운동 시작하기</button>
              </div>
              <div className="space-y-4 text-sm">
                <p><b>주소:</b> {currentGym.address}</p>
                <p><b>운영시간:</b> {currentGym.hours}</p>
        ...
        <div className={`absolute inset-0 bg-white z-[2000] transition-transform duration-500 ${activePage === 'workout' ? 'translate-y-0' : 'translate-y-full'}`}>
          <div className="p-10 text-center bg-gray-50">
            ...
            <div className="text-5xl font-black tracking-widest text-black">{formatTime(timer)}</div>
          </div>
          <div className="p-6">
             <h3 className="font-bold mb-4 text-black">운동 인증샷 AI 분석</h3>
             <div className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center text-gray-400 cursor-pointer">📷 사진 업로드</div>
             <button onClick={() => { clearInterval(timerInterval); setActivePage('home'); setTimer(0); alert('운동이 저장되었습니다!'); }} className="w-full bg-red-500 text-white py-4 rounded-xl font-bold mt-10">⏹ 운동 종료 및 저장</button>
          </div>
        </div>
